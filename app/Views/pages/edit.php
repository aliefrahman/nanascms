...
<div class="max-w-4xl mx-auto px-6 mt-10 relative z-10 pb-12 font-sans">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            <h1 class="font-display font-extrabold text-3xl text-white">Edit Halaman</h1>
            <p class="text-xs text-slate-500 mt-1">Perbarui detail, slug, atau konten dari halaman statis Anda.</p>
        </div>
    ...
    <div class="glass-card p-8 rounded-3xl border border-white/5">
        <form action="<?= route('/pages/update') ?>" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" id="title-input" value="<?= htmlspecialchars($page['title']) ?>"
                        required placeholder="Contoh: Tentang Kami, Syarat & Ketentuan"
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select name="status" id="status-select"
                            ...
                            <option value="draft" class="bg-dark-surface text-white" <?= $page['status'] === 'draft' ? 'selected' : '' ?>>Draft (Hanya Admin/Editor yang dapat melihat)</option>
                            <option value="published" class="bg-dark-surface text-white"
                                <?= $page['status'] === 'published' ? 'selected' : '' ?>>Published (Terbuka untuk Publik)
                            </option>
                        </select>
            ...
            <!-- URL Slug -->
            <div class="space-y-2">
                <label for="slug-input" class="text-xs font-semibold text-slate-300">URL Slug</label>
                <div class="flex items-center">
                    ...
                    <input type="text" name="slug" id="slug-input" value="<?= htmlspecialchars($page['slug']) ?>"
                        required placeholder="tentang-kami"
            ...
            <div class="space-y-2">
                ...
                <textarea name="content" id="content-input" rows="10" required
                    placeholder="Tulis konten halaman di sini..."
                    class="w-full bg-white/5 border border-white/10 focus:border-brand-primary/40 rounded-2xl py-3.5 px-4 text-sm text-white placeholder-slate-500 outline-none transition-all duration-300"><?= htmlspecialchars($page['content'] ?? '') ?></textarea>
            </div>

            <!-- Action Buttons -->
            <div class="pt-4 flex items-center justify-end space-x-4 border-t border-white/5">
                <a href="<?= route('/pages') ?>" class="px-6 py-3 rounded-full text-xs font-bold text-slate-400 border
                    border-white/5 hover:bg-white/5 hover:text-white transition-all">
                    Batal
                </a>
