...
<div class="max-w-7xl mx-auto px-6 mt-10 relative z-10 pb-12">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            <h1 class="font-display font-extrabold text-3xl text-white">Pengaturan Profil</h1>
            <p class="text-xs text-slate-500 mt-1">Kelola data personal, avatar profil, dan keamanan kata sandi Anda.
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-4 glass-card p-8 rounded-3xl border border-white/5 space-y-6 text-center">
            <h3 class="font-display font-bold text-lg text-white border-b border-white/5 pb-4">Preview Profil</h3>
            ...
            <div class="flex flex-col items-center space-y-4">
                ...
                <div class="relative group">
                    ...
                    <img id="avatar-preview-img" src="" alt="Avatar Preview"
                        class="w-24 h-24 rounded-full object-cover border-2 border-brand-primary/40 shadow-lg hidden">

                    <div id="avatar-preview-fallback"
                        class="w-24 h-24 rounded-full bg-linear-to-tr from-brand-secondary to-brand-primary flex items-center justify-center text-dark-bg font-extrabold text-3xl shadow-lg">
                        <span id="avatar-fallback-text">U</span>
                    </div>
                </div>

                <div class="space-y-1">
                    <h4 id="preview-display-name" class="font-display font-bold text-white text-lg">
                        <?= htmlspecialchars(!empty($user['fullname']) ? $user['fullname'] : $user['username']) ?>
                    </h4>
                    <p class="text-xs text-slate-500">@<span
                            id="preview-username"><?= htmlspecialchars($user['username']) ?></span></p>
                </div>

                <span
                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-brand-primary/10 text-brand-primary border border-brand-primary/20 uppercase tracking-wider text-[10px]">
                    <?= htmlspecialchars($user['role_name']) ?>
                </span>
            </div>

            <div class="space-y-3 text-left text-xs text-slate-400 bg-white/1 border border-white/5 rounded-2xl p-4">
                <div class="flex justify-between">
                    <span class="text-slate-500">Email:</span>
                    <span id="preview-email"
                        class="text-slate-300 font-medium"><?= htmlspecialchars($user['email']) ?></span>
                </div>
                <div class="flex justify-between">
                    <span class="text-slate-500">Terdaftar:</span>
                    <span class="text-slate-300 font-medium"><?= date('d F Y', strtotime($user['created_at'])) ?></span>
                </div>
            </div>
        </div>

        <!-- Right Column: Settings Form -->
        <div class="lg:col-span-8 glass-card p-8 md:p-10 rounded-3xl border border-white/5">
            <h3 class="font-display font-bold text-lg text-white border-b border-white/5 pb-4 mb-6">Ubah Profil</h3>
            ...
            <form action="<?= route('/profile/update') ?>" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <!-- Fullname Input -->
                    <div class="space-y-2">
                        <label for="fullname" class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Nama
                            Lengkap</label>
                        <div class="relative">
                            <i data-lucide="user"
                            ...
                            <input type="text" id="fullname" name="fullname"
                                ...
                                placeholder="Contoh: Nanas Studio">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <label for="username"
                            class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Username</label>
                        <div class="relative">
                            <i data-lucide="user"
                                class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500"></i>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <!-- Avatar URL Input -->
                    <div class="space-y-2">
                        <label for="avatar" class="text-xs font-semibold text-slate-400 uppercase tracking-wider">URL
                            Gambar Avatar</label>
                        <div class="flex gap-2">
                            ...
                            <button type="button" id="btn-media-picker"
                                class="px-4 py-4 rounded-xl border border-white/10 hover:border-brand-primary/30 bg-white/5 hover:bg-white/10 text-brand-primary text-xs font-bold transition-all duration-300 flex items-center space-x-1 focus:outline-none active:scale-95"
                                title="Pilih dari Media Sentral">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="password" id="password" name="password"
                            class="w-full pl-11 pr-5 py-4 rounded-xl bg-dark-bg/60 border border-white/10 focus:border-brand-primary/50 text-white placeholder-slate-600 focus:outline-none transition-colors text-sm"
                            placeholder="Ketik password baru jika ingin diubah (Min. 6 karakter)">
                    </div>
                </div>

                <button type="submit"
                    class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-4 rounded-full font-bold text-dark-bg bg-brand-primary hover:bg-white hover:scale-105 active:scale-95 transition-all duration-300 shadow-lg shadow-brand-primary/20 space-x-2 text-sm mt-4">
...
<div id="media-picker-modal"
    class="fixed inset-0 z-100 items-center justify-center bg-dark-bg/85 backdrop-blur-md transition-opacity duration-300"
    style="display:none">
    <div
        class="glass-card max-w-xl w-full p-6 md:p-8 rounded-4xl border border-white/5 shadow-2xl relative flex flex-col max-h-[80vh] mx-4">
        <!-- Close Button -->
        <button id="media-picker-close"
        ...
        <p class="text-xs text-slate-500 mb-6">Pilih gambar yang sudah diunggah sebelumnya atau unggah file baru
            langsung di sini.</p>

        <!-- Upload directly from modal -->
        <div id="modal-upload-container"
            class="border border-dashed border-white/10 hover:border-brand-primary/40 rounded-xl p-4 text-center cursor-pointer transition-colors relative mb-6">
            ...
            <p id="modal-upload-status" class="text-[9px] text-slate-500 mt-1">Hingga 5 MB (JPG, PNG, GIF, WEBP, SVG)
            </p>
        </div>

        <!-- Media Grid -->
        <div id="modal-media-grid"
            class="grid grid-cols-3 sm:grid-cols-4 gap-4 overflow-y-auto pr-1 flex-1 min-h-[200px]">
            <!-- Populated via JS -->
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateInitials() {
            const name = nameInput.value.trim() || usernameInput.value.trim() || 'U';
            fallbackText.textContent = name.substring(0, 1).toUpperCase();
        }

        function updateTextFields() {
            previewName.textContent = nameInput.value.trim() || usernameInput.value.trim() || 'Pengguna';
            previewUsername.textContent = usernameInput.value.trim() || 'username';
            previewEmail.textContent = emailInput.value.trim() || 'email@domain.com';
        }
        ...
        emailInput.addEventListener('input', updateTextFields);
