...
<div class="max-w-3xl mx-auto px-6 mt-10 relative z-10 pb-12">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            <h1 class="font-display font-extrabold text-3xl text-white">Tambah Kategori</h1>
            <p class="text-xs text-slate-500 mt-1 font-sans">Tambahkan klasifikasi konten baru ke dalam sistem.</p>
        </div>
        ...
        <form action="<?= route('/categories/store') ?>" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    <input type="text" 
                           ...
                           required 
                           autocomplete="off"
                           placeholder="Masukkan nama kategori (contoh: Tips & Trik)" 
                ...
                <div class="flex items-center space-x-1.5 text-[10px] text-slate-500 mt-1 select-none font-sans">
                    ...
                    <span class="font-mono text-brand-primary/80 bg-brand-primary/5 px-1.5 py-0.5 rounded border border-brand-primary/10">
                        /category/<span id="slug-preview">slug-kategori</span>
                    </span>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" 
                          ...
                          class="w-full bg-white/5 border border-white/10 focus:border-brand-primary/40 rounded-2xl py-3.5 px-4 text-sm text-white placeholder-slate-500 outline-none transition-all duration-300 resize-none"></textarea>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        nameInput.addEventListener('input', function() {
            let name = this.value;
            // JavaScript slug generator matching PHP's Category::generateSlug
            let slug = name.toLowerCase()
                           .replace(/[^a-z0-9\s-]/g, '') // remove special characters except spaces/dashes
                           .replace(/[\s-]+/g, '-')      // replace spaces/multiple dashes with single dash
